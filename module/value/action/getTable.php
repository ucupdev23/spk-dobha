...
$query = "SELECT * FROM weight_values JOIN users ON users.id = weight_values.employee_id WHERE users.deleted = 'no'";
...
$query .= " ORDER BY weight_values.id ASC LIMIT :start, :length";
...
if (count($result) > 0) {
    ...
    $recordsTotalQuery = "SELECT COUNT(*) FROM weight_values JOIN users ON users.id = weight_values.employee_id WHERE users.deleted = 'no'";
    if ($supervisor_id == 0) {
